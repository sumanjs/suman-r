...
exports.registerReporter = function (projectRoot, name) {
    ...
    try {
        ...
        su.vgt(5) && exports.log.info('reporter being loaded with the following path:');
        su.vgt(5) && exports.log.info(pth);
        reporterFn = require(pth);
    }
    catch (err) {
        try {
            pth = "suman-reporters/modules/" + name;
            su.vgt(5) && exports.log.info("reporter being loaded with the following resolved name '" + chalk.magenta(pth) + "'.");
            reporterFn = require(pth);
        }
        catch (err) {
